...
def projected_gradient_descent(
    ...
    i = 0
    adv_final = adv_x.clone()
    successfully_perturbed = torch.zeros(adv_final.shape[0])
    while i < nb_iter:
        adv_x = fast_gradient_method(model_fn, adv_x, eps_iter, norm, clip_min=clip_min, clip_max=clip_max, y=y, targeted=targeted, loss_fn=loss_fn, codewords=codewords)
        ...
        if early_stop:
            with torch.no_grad():
                ...
                ind = (y!=adv_predictions).nonzero(as_tuple=True)[0]
    ...
    if early_stop:
        ...
        adv_final = adv_x  # if dont want early stop replace adv_final with final perturbations at the max number if iterations.
