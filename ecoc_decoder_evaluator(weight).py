...
parser.add_argument('--times', default=2, type=int, metavar='N',
...
parser.add_argument('-weight_name', default='(CE+HL)cifar10-baseline-out100_best_checkpoint_1.pth.tar',
...
if __name__ == '__main__':
  ...
  for time in range(1, args.times+1):
    # if epoch % 20 == 0:
    ...
    if args.attack_type != 'None':
      ...
      att_acc.append(attacked_top1_accuracy.cpu())
  ...
  np_att_acc = np.array(att_acc)
  print('test avg: {0}, std: {1}'.format(round(np.average(test_acc),2), round(np.std(test_acc),2)))
  print('attacked avg: {0}, std: {1}'.format(round(np.average(np_att_acc),2), round(np.std(np_att_acc),2)))
  ...
  # print("best_epoch = {0}, best_val_acc = {1}, test_acc = {2}".format(best_epoch, best_val_acc, test_acc))