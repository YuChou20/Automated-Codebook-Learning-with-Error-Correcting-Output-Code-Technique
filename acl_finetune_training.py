...
def ecoc_decodering(features, codebook):
    ...
    codewcodebookords = codebook / torch.norm(codebook, dim=1, keepdim=True)
...
if __name__ == '__main__':
  ...
  # training & testing
  logging.basicConfig(filename=os.path.join('./runs/{0}'.format(args.folder_name), '({0})acl_finetune_training.log'.format(args.loss_type)), level=logging.DEBUG)
  logging.info('({0})acl_{1}'.format(args.loss_type, args.folder_name))
  ...
  checkpoint_name = '({0})acl_best_checkpoint_{1}.pth.tar'.format(args.loss_type, best_epoch)
  save_checkpoint({
      'epoch': best_epoch,
      'arch': args.arch,
