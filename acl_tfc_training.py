...
def generate_codeword(model, data, class_num=10, out_dim=2048):
  ...
  codebook = np.zeros((class_num, out_dim))
  ...
  for x, y in data:
    ...
    for c in range(class_num):
      ...
      codebook[c,:] += torch.sum(logits[yi],dim=0).data.cpu().numpy()
...
def calculate_cosine_similarity(features, codebook):
    codebook = torch.tensor(codebook, dtype=torch.float32, device=torch.device('cuda:0'),  requires_grad=True)
...
if __name__ == '__main__':
  ...
  # training & testing
  logging.basicConfig(filename=os.path.join('./runs/{0}'.format(args.folder_name), '({0})acl_tfc_training.log'.format(args.loss_type)), level=logging.DEBUG)
  logging.info('({0})acl_tfc{1}'.format(args.loss_type, args.folder_name))
  ...
  for epoch in range(1, args.epochs+1):
    ...
    for counter, (x_batch, y_batch) in enumerate(train_loader):
      ...
      pred_class = calculate_cosine_similarity(logits, codebook)
      ce = criterion(pred_class, y_batch) 
      mcsml = MCSM_loss(codebook)
      loss = ce + hl + rsl + mcsml
    ...
    codebook_test = generate_codeword(model, codeword_gen_loader, class_num=class_num, out_dim=code_dim)
    ...
    for counter, (x_batch, y_batch) in enumerate(val_loader):
      ...
      logits = calculate_cosine_similarity(model(x_batch), codebook_test)
  ...
  print("({0})acl_tfc best_epoch = {1}, best_val_acc = {2}, test_acc = {3}".format(args.loss_type, best_epoch, best_val_acc, test_acc))
