...
if __name__ == '__main__':
  ...
  # training & testing
  logging.basicConfig(filename=os.path.join('./runs/{0}'.format(args.folder_name), '({0})simclr_finetune_training.log'.format(cp_epoch)), level=logging.DEBUG)
  ...
  checkpoint_name = '({0})simclr_finetune_best_checkpoint_{1}.pth.tar'.format(args.loss_type, best_epoch)
  save_checkpoint({
      'epoch': best_epoch,
      'arch': args.arch,
