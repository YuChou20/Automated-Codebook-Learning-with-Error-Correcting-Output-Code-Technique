...
from cleverhans.attacks.fast_gradient_method_ecoc import fast_gradient_method
from cleverhans.attacks.projected_gradient_descent_ecoc import projected_gradient_descent
...
def FGSM(model, batch_size, test_loader, epsilon, norm, codewords=[]):
    '''
        FGSM attack
    ...
    for images, labels in test_loader: 
        ...
        x_ = fast_gradient_method(model.eval(), images, epsilon, norm, clip_min=0, clip_max=1, sanity_checks=False, targeted=False, y=labels, codewords=codewords) 
...
def PGD(model, batch_size, test_loader, epsilon, eps_step, max_iter, norm, loss, early_stop=False, codewords=[]): 
    '''
        PGD attack
                Parameters:
                        model: pytorch model under attack 
    ...
    for images, labels in test_loader: 
        ...
        x_ = projected_gradient_descent(model, images, epsilon, eps_step, max_iter, norm, clip_min=0, clip_max=1, sanity_checks=False, y=labels, targeted=False, early_stop=early_stop, loss_fn=loss, codewords=codewords) 
